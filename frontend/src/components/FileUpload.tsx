...
const getBestAlgorithm = (file: File): string => {
  ...
  if (
    ...
    ["txt", "json", "xml", "html", "css", "js", "ts"].includes(
      fileExtension || ""
    )
  ) {
    return "brotli";
...
const FileUpload: React.FC<FileUploadProps> = ({ onCompressionComplete }) => {
  ...
  const supportedFileTypes = [
    ...
    {
      type: "Text Files",
      extensions: ["txt", "json", "xml", "html", "css", "js", "ts"],
      icon: <CodeIcon />,
    },
    {
      type: "Other",
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/compression/compress`,
        {
          method: "POST",
          headers: {
      ...
      const compressedUrl = result.data.downloadUrl
      ...
      const metadataResponse = await fetch(
        ...
        }/api/compression/metadata/${encodeURIComponent(selectedFile.name)}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
